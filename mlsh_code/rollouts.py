...
def traj_segment_generator(pi, sub_policies, gs_policy, env, macrolen, horizon, stochastic, args):
    replay = args.replay
    t = 0
    ...
    gs_vpreds = np.zeros(horizon, 'float32')
    news = np.zeros(horizon, 'int32')
    ticksSinceLastRewardList = np.zeros(horizon, 'float32')
    ticksTillNextRewardList = np.zeros(horizon, 'float32')
    ...
    while True:
        ...
        if (t > 0) and (t % horizon == 0):
            ...
            # print(total)
            # total = [0,0]
            dicti = {"ob" : obs, "rew" : rews, "vpred" : vpreds, "gs_vpreds" : gs_vpreds , "new" : news, "ac" : acs, "ep_rets" : ep_rets, "ep_lens" : ep_lens, "macro_ac" : macro_acs, "macro_vpred" : macro_vpreds,  "steps" : ticksTillNextRewardList}
        ...
        obs[i] = ob
        vpreds[i] = vpred
        gs_vpreds[i] = gs_vpred
        news[i] = new
        acs[i] = ac
        ticksSinceLastRewardList[i] = ticksSinceLastReward
        if t % macrolen == 0:
        ...
        ob = ob.flatten()
        rews[i] = rew + (0.0001*(old_gs_vpred-gs_vpred))
        ...
        if replay:
            ...
            pass
        if rew > 0:
            for n in range(t-ticksSinceLastReward,t):
                ticksTillNextRewardList[n % horizon] = ticksSinceLastReward-ticksSinceLastRewardList[n % horizon]
            ticksSinceLastReward = 0
        cur_ep_ret += rew
        cur_ep_len += 1
        ...
        if new:
            ...
            cur_ep_len = 0
            old_gs_vpred = 0
            print("\t\tenv.reset, new:%s, macorlen_cond:%s, t:%d, ticksSinceLastReward: %d" % (new,((t+1) % macrolen == 0),t,ticksSinceLastReward))
            ob = env.reset().flatten()
            ticksSinceLastReward = -1
        old_gs_vpred = gs_vpred
        t += 1
        ticksSinceLastReward += 1
...
def add_advantage_macro(seg, macrolen, gamma, lam):
    ...
    seg["macro_steps"] = seg["steps"][0::macrolen]
